...
//			190030061
			...
			if(m == 1) {
				...
				while(rs.next()) {
							i++;
							File f=new File("D:\\Users\\prade\\Desktop\\Animal\\animal " + i + ".jpg");
						    FileOutputStream image0=new FileOutputStream(f);
						      blob=rs.getBlob("image");
						       b = blob.getBytes(1, (int)blob.length());
						      image0.write(b);
						      
						      File f0=new File("D:\\Users\\prade\\Desktop\\Animal\\animal " + i + ".txt");
						      FileOutputStream file0 = new FileOutputStream(f0);
						      clob = rs.getClob("description_file");
			...
			int n = s.nextInt();
			 if(n == 2) {
				...
				while(rs1.next()) {
					j++;
					File f1=new File("D:\\Users\\prade\\Desktop\\Plant\\plant " + j + ".jpg");
					FileOutputStream image=new FileOutputStream(f1);
					b1=rs1.getBlob("image");
					by = b1.getBytes(1, (int)b1.length());
					image.write(by);
					
					File f2=new File("D:\\Users\\prade\\Desktop\\Plant\\plant " + j + ".txt");
					FileOutputStream file1 = new FileOutputStream(f2);
